...
> `3K-final`: 3072x1728 at 30 fps CFR, 8-bit 4:2:0 Rec.709 SDR, AVC/H.264 and AAC-LC 48 kHz stereo at the mandatory 192 kb/s baseline in fast-start MP4. This profile is **required** on every named baseline machine; 320 kb/s is a separately reported optional capability. The implementation records the encoder-selected profile/level and hardware/software path. If the baseline profile cannot be produced or independently decoded on either OS baseline, the slice fails and ADR-001 is reopened. The lossless oracle intermediate is a supplemental diagnostic artifact, never a pass condition.
