...
- Persistence: `ProjectStore v1`, a local single-writer bundled SQLite >= 3.51.3 database plus an external disposable cache directory. The store verifies its SQLite version/compile options, `journal_mode`, and `synchronous=FULL` on open. A command is acknowledged only after command, immutable revision, and head commit in one transaction. The live database, WAL, and SHM remain one unit until verified checkpoint/close; one worker owns writes and cancel-safe compaction.
